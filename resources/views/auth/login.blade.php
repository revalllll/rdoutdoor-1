...
<head>
  ...
  <style>
    ...
    body::before {
      ...
      background: rgba(248,250,252,0.32); /* overlay agar form tetap terbaca */
      z-index: 0;
    }
    .bg-white {
      position: relative;
...
<body class="flex items-center justify-center">
  ...
  <div class="bg-white p-8 rounded-lg shadow-xl w-full max-w-md login-animate" style="margin: 48px auto; background: rgba(255,255,255,0.65); backdrop-filter: blur(4px); box-shadow: 0 8px 32px rgba(23,99,92,0.13);">
    ...
    <div class="mb-6">
      <a href="/auth/google" class="flex items-center justify-center w-full py-2 px-4 btn-green text-white">
    ...
    <form method="POST" action="{{ route('login') }}">
      ...
      <div class="mb-4">
        ...
        <input type="email" name="email" class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus-green" required autofocus placeholder="Masukkan Email">
      </div>
      ...
      <div class="flex items-center justify-between mb-6">
        <div class="flex items-center">
          <input type="checkbox" name="remember" id="remember" class="h-4 w-4 border-gray-300 text-green-600 focus-green rounded">
          <label for="remember" class="ml-2 text-sm text-gray-600">Ingat saya</label>
        </div>
        @if (Route::has('password.request'))
          <a href="{{ route('password.request') }}" class="text-sm text-green-link font-medium">Lupa kata sandi?</a>
        @endif
      </div>
    ...
    <p class="text-center text-sm text-gray-600">
      Belum punya akun? 
      <a href="{{ route('register') }}" class="text-green-link font-medium">Daftar di sini</a>
    </p>
  </div>

  <script>
    document.addEventListener('DOMContentLoaded', function() {
      // Autofocus ke input email
      ...
      // Loader hiking saat submit login (tanpa delay) + auto hide on error
      document.addEventListener('DOMContentLoaded', function() {
        var loginForm = document.querySelector('form[action*="login"]');
